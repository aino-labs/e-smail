...
func (s *Service) SendDraft(ctx context.Context, in SendDraftInput) (*SendEmailResult, error) {
	...
	// Черновик мог пролежать сколько угодно: согласие получателя проверяем
	// заново на момент отправки, а не на момент сохранения.
	if d.IsAnonymous {
		...
		if len(rejected) > 0 {
			// Черновик уже существует — новый не плодим, возвращаем этот же.
